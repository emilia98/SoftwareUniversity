    ...
    class SocialMediaPosts
    {
        static void Main()
        {
            ...
            foreach (var post in postInfo)
            {
                ...
                {
                    hasComments = true;
                    List<string> commentatorsList = commentsInfo[postName].Keys.ToList();
                    List<string> textList = commentsInfo[postName].Values.ToList();
                   
                    for (int index = 0; index < commentatorsList.Count; index++)
                    {
                        string commentator = commentatorsList[index];
