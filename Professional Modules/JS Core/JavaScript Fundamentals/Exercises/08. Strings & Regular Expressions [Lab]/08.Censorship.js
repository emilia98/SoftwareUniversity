function censorText(str, arr) {

    for (let word of arr) {
        let startingIndex = str.indexOf(word);
        let replacingChars = "-".repeat(word.length);

        while(startingIndex > -1){
            str = str.replace(word, replacingChars);
            startingIndex = str.indexOf(word);
        }
    }
    console.log(str);
}
...
censorText('roses are red, roses are blue', ['roses', 'red']);
